...
export function Dashboard() {
  ...
  // Focus anchor for controls that unmount themselves — see the Retry button.
  ...
  const { summary, trend, categories, loading, fetching, error, refetch } =
  ...
  /* ── Render ── */
  // True for a same-key refetch with data already on screen — the SSE
  // invalidate path. NOT a period change: that mints a new query key, so
  // `loading` is true and the content below is a skeleton instead.
  const refetching = fetching && !loading;

  // ONE return, deliberately. `loading` and `error` used to be early returns
  // that re-rendered their own header — a bare `<Skeleton>` row and a plain
  // `<h1>Dashboard</h1>` — which meant a period change UNMOUNTED the Select
  // the user had just operated.
  //
  // That is the whole B34 defect: Radix restores focus from a `setTimeout(0)`
  // in FocusScope's unmount cleanup, a full macrotask after the commit that
  // closed the menu, and by then `context.trigger` is a detached node.
  // Focusing a detached node is a no-op, and Radix `preventDefault()`s the
  // event afterwards — which also cancels FocusScope's own
  // `previouslyFocusedElement ?? document.body` fallback. A keyboard or switch
  // user who changed the month was dropped to the top of the document.
  //
  // So the header renders in exactly one place and no state may swap it. Keep
  // it that way: a branch that returns its own header brings the drop back,
  ...
  return (
    <div className={cn('flex flex-col gap-6 transition-opacity duration-200', refetching && 'opacity-60')}>
      {/* Header — see the note above: rendered by every state, swapped by none. */}
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          {/* `tabIndex={-1}`: a programmatic focus anchor, not a tab stop.
              Same idiom as the Trash and Transactions headings. Retry is the
              one control here that unmounts itself by succeeding, so it is
              the one that needs somewhere to hand focus to. */}
          <h1
            ref={headingRef}
            tabIndex={-1}
      ...
      {/* Content only. The period controls above sit outside this swap. */}
      {loading ? (
        <>
        ...
        <Alert variant="destructive">
          <AlertDescription className="flex items-center justify-between">
            {error}
            {/* Refetches the period's trio; it does NOT reload the page. One
                failed query is not a reason to tear down the client, drop
                every other cached query and lose the user's place — and the
                ...
                exact drop this page was just fixed for. A double-press is
                harmless anyway — TanStack dedupes concurrent fetches for one
                key, and the skeleton already reports the in-flight state. */}
            <Button
              variant="outline"
              size="sm"
              onClick={() => {
