...
- **Amber TEXT is not the icon's amber.** The icon keeps `text-amber-500` in both themes (a non-text element, held to 3:1, and never the only signal). Body-sized text beside it takes `ATTENTION_TEXT_CLASS` from `lib/attention.ts` — `text-amber-600 dark:text-amber-500` — because `amber-500` on the light theme's white measures ≈2.1:1 at 14px, against the 4.5:1 text needs. Import this rather than writing the pair out; it is one edit the day a `--warning` token lands.
