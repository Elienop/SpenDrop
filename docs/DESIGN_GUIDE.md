...
- **Amber TEXT is not the icon's amber.** Body-sized text takes `ATTENTION_TEXT_CLASS` from `lib/attention.ts` — `text-amber-700 dark:text-amber-500`. Computed against the surfaces these strings sit on (light `--card`/`--background` are pure white; the dark `--card` is `240 4% 9%`): amber-500 is **2.15:1** on white and 8.41:1 on the dark card, amber-600 is **3.19:1** (it clears the 3:1 for non-text, NOT the 4.5:1 AA needs for body text), amber-700 is **5.02:1** on white and 3.60:1 on the dark card. Neither shade works in both themes, which is why the token is a pair. The ICON keeps `text-amber-500` throughout: non-text, held to 3:1, and never the only signal — the sentence beside it says the same thing. Import the constant rather than writing the pair out; it is one edit the day a `--warning` token lands.
- **An aggregate bar belongs OUTSIDE the table it describes.** A bar rendered as a `colSpan` cell inherits the table's width, so on a phone its buttons sit off the right edge of a horizontally-scrolling table (measured at 360: a button at x 328–590 in a 246px scroller), and as a tbody row it scrolls out of view as soon as the user reaches the rows it is about. Put it in the card above the scroll container; keep GROUP headers in the table, since those label the rows immediately below them.
