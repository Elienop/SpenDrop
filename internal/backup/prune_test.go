...
func TestPrune_KeepsNewestDaily(t *testing.T) {
	...
	kept, removed, err := Prune(dir, now, 3, 0, 0)
	if err != nil {
...
func TestPrune_RemovesSidecarCompanion(t *testing.T) {
	...
	_, removed, err := Prune(dir, now, 2, 0, 0)
	if err != nil {
		t.Fatalf("Prune: %v", err)
	}
	if len(removed) != 3 {
...
func TestPrune_IgnoresUnrelatedFiles(t *testing.T) {
	...
	if _, _, err := Prune(dir, now, 1, 0, 0); err != nil {
		t.Fatalf("Prune: %v", err)
	}
...
func TestPrune_40DaySimulation(t *testing.T) {
	...
	kept, removed, err := Prune(dir, now, 7, 4, 12)
	if err != nil {
		t.Fatalf("Prune: %v", err)
	}
...
func TestPrune_EmptyDir(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	kept, removed, err := Prune(dir, time.Now(), 7, 4, 12)
	if err != nil {
		t.Fatalf("Prune: %v", err)
	}
...
func TestPrune_MissingDir(t *testing.T) {
	t.Parallel()
	if _, _, err := Prune(filepath.Join(t.TempDir(), "does-not-exist"), time.Now(), 1, 0, 0); err == nil {
