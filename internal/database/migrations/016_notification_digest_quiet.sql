...
-- digest_mode 'off'|'daily' gates the daily rollup ticker. quiet_start/quiet_end
-- are 'HH:MM' ('' = unset/never quiet); quiet_tz is an IANA zone (default UTC) so
-- the window is evaluated in the household's local time. quiet_allow_over_budget
-- lets state alerts pierce quiet hours by default. last_digest_at (nullable) is
-- the restart-safe cursor for "what changed since the last digest".
