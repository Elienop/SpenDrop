...
**Refunds are negative amounts.** Enter a refund against the same expense category as the original purchase and everything nets it out — spend $50 on groceries, get $20 back, and Groceries reads $30 on the dashboard, in every budget bar and report tab, in the Excel export, and in the over-budget alert (a refund that drops a category back under its limit clears the alert). A refunded row shows its amount as `+$20.00` with a quiet "Refund" note so it cannot be mistaken for income. Foreign-currency refunds work the same way, and newly created foreign rows now record the exchange rate they were booked at.
...
| `month_remaining` | number | `month_budget - month_spent`; negative when over budget. With no budget row set, `month_budget` is `0` so `month_remaining` equals `-month_spent` — normally negative, though a month whose refunds exceed its spending makes `month_spent` itself negative and this positive. |
